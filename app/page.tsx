...
function GameApp({gameId}:{gameId:string}){
  ...
  const [syncKey,setSyncKey]=useState(0); const [lastSync,setLastSync]=useState<Date|null>(null);
  ...
  const [editAnnouncement,setEditAnnouncement]=useState(""); const [editBingoReward,setEditBingoReward]=useState(0); const [editCompleteReward,setEditCompleteReward]=useState(0); const [rewardMessage,setRewardMessage]=useState("");
  useEffect(()=>subscribeGame(gameId,value=>{setGame(value);setLastSync(new Date())}),[gameId,syncKey]);
  useEffect(()=>subscribeSubmissions(gameId,value=>{setPosts(value);setLastSync(new Date())}),[gameId,syncKey]);
  ...
  useEffect(()=>{if(game){setEditStart(game.startsAt?localDateTime(new Date(game.startsAt)):defaultStart());setEditEnd(game.endsAt?localDateTime(new Date(game.endsAt)):defaultEnd());setEditAnnouncement(game.announcement??"");setEditBingoReward(game.bingoReward??0);setEditCompleteReward(game.completeReward??0)}},[game]);
  ...
  async function saveRewards(){try{await updateGame(gameId,{announcement:editAnnouncement.trim().slice(0,200),bingoReward:Math.max(0,Math.floor(editBingoReward)),completeReward:Math.max(0,Math.floor(editCompleteReward))});setRewardMessage("メッセージと賞金を保存しました")}catch(e){setRewardMessage(e instanceof Error?e.message:"保存できませんでした")}}
  ...
  return <main className="shell"><header><div className="brand"><span className="logo">M</span><div><small>みんなで集める思い出</small><h1>{game.title}</h1></div></div><div className="header-actions"><div className="person" style={{"--person":current.color} as React.CSSProperties}><span>{initial(current.name)}</span>{current.name}</div><button className="settings-button" onClick={()=>setTab("settings")} aria-label="設定を開く" title="設定">⚙</button></div></header>
    <section className="hero"><div><span className="eyebrow">FAMILY QUEST</span><h2>今日の小さな冒険へ</h2><p>一つずつ、みんなの宝物に。</p></div><div className="hero-stamp"><b>{done.length}</b><span>/ 25</span><small>達成</small></div></section>{(game.announcement||game.bingoReward||game.completeReward)&&<section className="reward-banner">{game.announcement&&<p>📣 {game.announcement}</p>}<div>{Boolean(game.bingoReward)&&<b>1列ビンゴで <strong>{game.bingoReward?.toLocaleString()}円！</strong></b>}{Boolean(game.completeReward)&&<b>コンプリートで <strong>{game.completeReward?.toLocaleString()}円！</strong></b>}</div></section>}{hasPeriod&&<div className={`period-status ${phase}`}>{periodText}</div>}<div className="sync-status"><span>● {lastSync?`${lastSync.toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"})} 同期済み`:"同期中…"}</span><button onClick={()=>setSyncKey(v=>v+1)}>今すぐ更新</button></div>
