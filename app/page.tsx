...
import {
  addPhotoComment, approveMission, cancelSubmission, claimPrimaryEditor, createGame, photoBytesToUrl,
...
function GameApp({gameId,onSwitchGame,onCreateGame}:{gameId:string;onSwitchGame:(id:string)=>void;onCreateGame:()=>void}){
  ...
  useEffect(()=>{if(game&&me===game.participants[0]?.id&&!game.primaryEditorUid)claimPrimaryEditor(gameId).catch(()=>{})},[game,me,gameId]);
