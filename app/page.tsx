...
function GameApp({gameId}:{gameId:string}){
  ...
  return <main className="shell"><header><div className="brand"><span className="logo">M</span><div><small>みんなで集める思い出</small><h1>{game.title}</h1></div></div><button className="person" style={{"--person":current.color} as React.CSSProperties} onClick={()=>setTab("settings")}><span>{initial(current.name)}</span>{current.name}</button></header>
    ...
    {tab==="settings"&&<section className="page"><div className="section-title"><div><small>SHARE</small><h2>家族を招待する</h2></div></div><article className="share-card"><b>このゲーム専用の共有URL</b><small>{location.href}</small><button onClick={shareGame}>共有する・URLをコピー</button>{shareMessage&&<strong className="share-message">{shareMessage}</strong>}</article><p className="settings-note">このURLを受け取った人は、最初に自分の名前を一度だけ選びます。</p></section>}
