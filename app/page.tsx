...
function GameApp({gameId}:{gameId:string}){
  ...
  return <main className="shell"><header><div className="brand"><span className="logo">M</span><div><small>みんなで集める思い出</small><h1>{game.title}</h1></div></div><button className="person" style={{"--person":current.color} as React.CSSProperties} onClick={()=>setTab("settings")}><span>{initial(current.name)}</span>{current.name}</button></header>
    <section className="hero"><div><span className="eyebrow">FAMILY QUEST</span><h2>今日の小さな冒険へ</h2><p>一つずつ、みんなの宝物に。</p></div><div className="hero-stamp"><b>{done.length}</b><span>/ 25</span><small>達成</small></div></section><div className="sync-status"><span>● {lastSync?`${lastSync.toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"})} 同期済み`:"同期中…"}</span><button onClick={()=>setSyncKey(v=>v+1)}>今すぐ更新</button></div>
