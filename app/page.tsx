...
function GameApp({gameId,onSwitchGame,onCreateGame}:{gameId:string;onSwitchGame:(id:string)=>void;onCreateGame:()=>void}){
  ...
  useEffect(()=>{if(!game)return;const canManage=auth.currentUser?.uid===game.ownerUid;if(!canManage&&tab==="settings")setTab("bingo")},[game,tab]);
  useEffect(()=>{if(tab!=="album")return;const ids=posts.filter(post=>post.status==="approved").map(post=>post.id).sort();const key=`${gameId}:${ids.join(",")}`;if(albumLoadKey.current===key)return;albumLoadKey.current=key;loadAlbumComments(gameId,ids).then(setAlbumComments).catch(()=>setAlbumComments({}))},[tab,posts,gameId]);
  ...
  const canEdit=auth.currentUser?.uid===game.ownerUid;
